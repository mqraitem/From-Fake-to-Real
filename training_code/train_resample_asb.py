...
class ResNet50Trainer(TrainerBase):    
    def __init__(self, root_dir, transform=None, opt=None):
        ...
        num_gen, _ = dataset_train_balance.get_gen_ratio(False)
        opt.num_per_group = num_gen // len(self.dataset_train.GROUPS) 
        self.dataset_train.load_gen_data()
        num_gen, _ = self.dataset_train.get_gen_ratio()
        ...
        weights = self.dataset_train.group_counts_dro_real_gen()/np.sum(self.dataset_train.group_counts_dro_real_gen())
        weights = 1/weights
        weights = weights[self.dataset_train.group_idx_real_gen]
