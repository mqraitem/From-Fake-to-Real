...
class ResNet50(nn.Module):
    def __init__(self, num_classes):
        ...
        print(f'FCResNet50 - num_classes: {self.num_classes} pretrained: {True}')

    def reinit_linear_layer(self): 
        self.fc = nn.Linear(self.embed_size, self.num_classes)
...
def get_datasets_dir(dataset):
    dataset_dirs = open("datasets_dirs.txt", "r").readlines() 
    dataset_dirs = [x.strip().split(":") for x in dataset_dirs]
    dataset_dirs = {x[0]: x[1] for x in dataset_dirs}
    return dataset_dirs[dataset]

def compute_stats(outputs, targets, biases, GROUPS, take_argmax=True):
    ...
    for target_value, bias_value in GROUPS:
        ...
        idx = np.where((targets == target_value) & (biases == bias_value))[0]
        accuracies.append(np.sum(outputs[idx] == targets[idx]) / len(idx))
...
class DROLossComputer:
    def __init__(self, criterion, is_robust, n_groups, group_counts, alpha=None, gamma=0.1, adj=None, min_var_weight=0, step_size=0.01, normalize_loss=False, btl=False):
        ...
        self.group_counts = torch.Tensor(group_counts).cuda()
